(async function () {
  ...
  const mapContainer = d3.select("#svgd_conclusion_map_container");
  ...
  const stations_svgd = await fetch("./data/seattle/svgd_all.json")
  .then((response) => response.json())
  .then((d) => d);
  ...
  var maxiters = Number(d3.select("#timeslide_conclusion").attr("max"))
  var iters = [...Array(maxiters).keys()]

  points
    .selectAll("circle.conclusion_svgd_station")
    .data(stations_svgd)
    .join("circle")
    .attr("class", "conclusion_svgd_station")
    .attr("transform", (d) => {
  ...
  d3.select("#timeslide_conclusion").on("input", function() {
        update(+this.value);
    });
  ...
  function update(value) {
      ...
      d3.selectAll(".conclusion_svgd_station")
