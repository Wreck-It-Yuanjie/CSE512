(async function () {
  ...
  var mouseleave = function (event, d) {
    ...
    if (clickedStation !== d.properties.station_id) {
      d3.select(this)
        .style("stroke", "black")
        .style("r", 6)
  ...
  var mouseclick = function (event, d) {
    clickedStation = d.properties.station_id;
  ...
  const traffic = ["Borrowed out from the station", "Returned to the station", 
                    "Both borrowed out and returned to the station"]
  const traffic_color = [["#56B4E9", "#56B4E9"], ["#FFE200", "#FFE200"], ["#B9B914", "#31DAC7"]]
  function lineLegend(container) {
    ...
    const symbols = entries
      ...
      .attr("width", 4)
      .attr("height", 2)
      .attr("fill", (d) => color_trffic(d)[0])
      .attr("fill-opacity", 0.7);
    
    // counter the mixed color palette
    const symbols2 = entries
        .append("rect")
        .attr('x', 4)
        .attr('y', 0)
        .attr("width", 4)
        .attr("height", 2)
        .attr("fill", (d) => color_trffic(d)[1])
        .attr("fill-opacity", 0.7);
  ...
  const legend_line = map
    .append("g")
    .attr("transform", "translate(20, 710)")
    .call(lineLegend);
