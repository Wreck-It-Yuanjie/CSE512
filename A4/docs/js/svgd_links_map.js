(async function () {
  ...
  const mapContainer = d3.select("#svgd_links_map_container");
  ...
  const stations_svgd = await fetch("./data/seattle/link_stations_svgd.json")
  .then((response) => response.json())
  .then((d) => d);
  ...
  var maxiters = Number(d3.select("#timeslide_links").attr("max"))
  var iters = [...Array(maxiters).keys()]

  points
    .selectAll("circle.links_svgd_station")
    .data(stations_svgd)
    .join("circle")
    .attr("class", "links_svgd_station")
    .attr("transform", (d) => {
  ...
  d3.select("#timeslide_links").on("input", function() {
        update(+this.value);
    });
  ...
  function update(value) {
      ...
      d3.selectAll(".links_svgd_station")
