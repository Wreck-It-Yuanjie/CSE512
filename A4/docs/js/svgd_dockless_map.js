(async function () {
  ...
  const mapContainer = d3.select("#svgd_dockless_map_container");
  ...
  const stations_svgd = await fetch("./data/seattle/dockless_end_aggregated_svgd.json")
  .then((response) => response.json())
  .then((d) => d);
  ...
  var maxiters = Number(d3.select("#timeslide_dockless").attr("max"))
  var iters = [...Array(maxiters).keys()]

  points
    .selectAll("circle.dockless_svgd_station")
    .data(stations_svgd)
    .join("circle")
    .attr("class", "dockless_svgd_station")
    .attr("transform", (d) => {
  ...
  d3.select("#timeslide_dockless").on("input", function() {
        update(+this.value);
    });
  ...
  function update(value) {
      ...
      d3.selectAll(".dockless_svgd_station")
