(async function () {
  ...
  const mapContainer = d3.select("#svgd_food_map_container");
  ...
  const stations_svgd = await fetch("./data/seattle/POI_food_svgd.json")
  .then((response) => response.json())
  .then((d) => d);
  ...
  var maxiters = Number(d3.select("#timeslide_food").attr("max"))
  var iters = [...Array(maxiters).keys()]

  points
    .selectAll("circle.food_svgd_station")
    .data(stations_svgd)
    .join("circle")
    .attr("class", "food_svgd_station")
    .attr("transform", (d) => {
  ...
  d3.select("#timeslide_food").on("input", function() {
        update(+this.value);
    });
  ...
  function update(value) {
      ...
      d3.selectAll(".food_svgd_station")
