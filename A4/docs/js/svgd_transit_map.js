(async function () {
  ...
  const mapContainer = d3.select("#svgd_transit_map_container");
  ...
  const stations_svgd = await fetch("./data/seattle/transit_svgd.json")
  .then((response) => response.json())
  .then((d) => d);
  ...
  var maxiters = Number(d3.select("#timeslide_transit").attr("max"))
  var iters = [...Array(maxiters).keys()]

  points
    .selectAll("circle.transit_svgd_station")
    .data(stations_svgd)
    .join("circle")
    .attr("class", "transit_svgd_station")
    .attr("transform", (d) => {
  ...
  d3.select("#timeslide_transit").on("input", function() {
        update(+this.value);
    });
  ...
  function update(value) {
      ...
      d3.selectAll(".transit_svgd_station")
