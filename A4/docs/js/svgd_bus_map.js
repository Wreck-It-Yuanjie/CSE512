(async function () {
  ...
  const mapContainer = d3.select("#svgd_bus_map_container");
  ...
  const stations_svgd = await fetch("./data/seattle/POI_bus_svgd.json")
  .then((response) => response.json())
  .then((d) => d);
  ...
  var maxiters = Number(d3.select("#timeslide_bus").attr("max"))
  var iters = [...Array(maxiters).keys()]

  points
    .selectAll("circle.bus_svgd_station")
    .data(stations_svgd)
    .join("circle")
    .attr("class", "bus_svgd_station")
    .attr("transform", (d) => {
  ...
  d3.select("#timeslide_bus").on("input", function() {
        update(+this.value);
    });
